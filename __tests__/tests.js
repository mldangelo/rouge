...
describe("Utility Functions", () => {
  ...
  describe("lcs", () => {
    ...
    test("should return [w1, w3, w5] for [w1, w2, w3, w4, w5] and [w1, w3, w8, w9, w5]", () => {
      expect(
        lcs(["w1", "w2", "w3", "w4", "w5"], ["w1", "w3", "w8", "w9", "w5"]),
      ).toEqual(["w1", "w3", "w5"]);
    });
  });
  ...
  describe("sentenceSegment", () => {
    ...
    test("should split simple periods", () => {
      expect(ss("Hello World. My name is Jonas.")).toEqual([
        "Hello World.",
        "My name is Jonas.",
      ]);
    });
    ...
    test("should split end-of-sentence exclamation marks", () => {
      expect(ss("There it is! I found it.")).toEqual([
        "There it is!",
        "I found it.",
      ]);
    });
    ...
    test("should not split singleton uppercase abbreviations", () => {
      expect(ss("My name is Jonas E. Smith.")).toEqual([
    ...
    test("should not split two letter lowercase abbreviations in the middle of a sentence", () => {
      expect(ss("Were Jane and co. at the party?")).toEqual([
    ...
    test("should not split two letter uppercase abbreviations in the middle of a sentence", () => {
      expect(
        ss("They closed the deal with Pitt, Briggs & Co. at noon."),
      ).toEqual(["They closed the deal with Pitt, Briggs & Co. at noon."]);
    });
    ...
    test("should split two letter uppercase abbreviations at the end of a sentence", () => {
      expect(
        ss("They closed the deal with Pitt, Briggs & Co. It closed yesterday."),
      ).toEqual([
    ...
    test("should not split two letter (prepositive) abbreviations", () => {
      expect(ss("I can see Mt. Fuji from here.")).toEqual([
    ...
    test("should not split numbers as a non-sentence boundary", () => {
      expect(ss("She has $100.00 in her bag.")).toEqual([
    ...
    test("should split email addresses as a sentence boundary", () => {
      expect(
        ss("Her email is Jane.Doe@example.com. I sent her an email."),
      ).toEqual(["Her email is Jane.Doe@example.com.", "I sent her an email."]);
    });
    ...
    test("should split double exclamation points", () => {
      expect(ss("Hello!! Long time no see.")).toEqual([
        "Hello!!",
        "Long time no see.",
      ]);
    });
    ...
    test("should not split errant newlines in the middle of sentences (PDF)", () => {
      ...
      ).toEqual(["This is a sentence cut off in the middle because pdf."]);
    });
  ...
  describe("treeBankTokenize", () => {
    ...
    test("should handle double quotation marks", () => {
      expect(
        tbt('"We beat some pretty good teams to get here," Slocum said.'),
      ).toEqual([
        "``",
        "We",
        "beat",
